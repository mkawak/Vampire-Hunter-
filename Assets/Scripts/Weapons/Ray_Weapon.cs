...
public class Ray_Weapon : Weapon
{
...
/*
    int range = 10; // Radius of range circle
    public override void Fire() {
        int angleInd = 0;
        
        for (int i = 0; i < numProjectiles; i++) {
            int targetx = Random.Range(-range, range);
            int targety = Random.Range(-range, range);
            
            Projectile currProj = Instantiate(projectile, transform.position + new Vector3(targetx, targety, 0), Quaternion.identity);
